...
playersList?.addEventListener("click", (event) => {
  ...
  const player = loadedPlayers.find(
    (item) => item.uid === playerUid
  );

  if (player) {
    openCoinsModal(player);
  }
});
...
function openCoinsModal(player) {
  closeCoinsModal();

  const modal = document.createElement("div");
  modal.id = "coinsAdminModal";
  modal.className = "coins-admin-overlay";

  modal.innerHTML = `
    <section class="coins-admin-modal">
      <button
        id="closeCoinsModalButton"
        class="coins-modal-close"
        type="button"
        aria-label="Cerrar"
      >
        ✕
      ...
      <div class="coins-modal-icon">🪙</div>

      <p class="coins-modal-label">
        Administrar monedas
      </p>

      <h2>${escapeHtml(player.name)}</h2>

      <div class="coins-current-balance">
      ...
      <label class="coins-amount-label" for="coinsAmountInput">
        Cantidad de monedas
      </label>

      <input
        id="coinsAmountInput"
        class="coins-amount-input"
        type="number"
        min="1"
        step="1"
        inputmode="numeric"
      ...
      <div class="coins-modal-actions">
        ...
        <button
          id="removeCoinsButton"
          class="coins-remove-button"
          type="button"
        >
          ➖ Quitar monedas
        </button>
      ...
      <p
        id="coinsModalMessage"
        class="coins-modal-message"
      ></p>
    </section>
  `;
  ...
  document
    .getElementById("closeCoinsModalButton")
  ...
  document
    .getElementById("addCoinsButton")
    ?.addEventListener("click", () => {
      changePlayerCoins(player.uid, "add");
    });

  document
    .getElementById("removeCoinsButton")
    ?.addEventListener("click", () => {
      changePlayerCoins(player.uid, "remove");
    });

  document
    .getElementById("coinsAmountInput")
    ?.focus();
}

async function changePlayerCoins(playerUid, operation) {
  const amountInput =
    document.getElementById("coinsAmountInput");

  const messageElement =
    document.getElementById("coinsModalMessage");

  const addButton =
    document.getElementById("addCoinsButton");

  const removeButton =
    document.getElementById("removeCoinsButton");
  ...
  if (newCoins < 0) {
    showCoinsMessage(
      "No puedes dejar el saldo en números negativos.",
      "error"
    );
    return;
  }
  ...
  if (messageElement) {
    ...
    messageElement.className =
      "coins-modal-message loading";
  }

  try {
    await updateDoc(
      doc(db, USERS_COLLECTION, playerUid),
      {
        coins: newCoins,
        coinsUpdatedAt: new Date()
      }
    );
    ...
    const searchInput =
      document.getElementById("playersSearchInput");
    ...
    showCoinsMessage(
      operation === "add"
        ? `${formatNumber(amount)} monedas agregadas correctamente.`
        : `${formatNumber(amount)} monedas retiradas correctamente.`,
      "success"
    );
    ...
    console.error(
      "Error al actualizar monedas:",
      error
    );

    showCoinsMessage(
      error.message ||
      "No fue posible actualizar las monedas.",
      "error"
    );
...
function showCoinsMessage(message, type) {
  const messageElement =
    document.getElementById("coinsModalMessage");
